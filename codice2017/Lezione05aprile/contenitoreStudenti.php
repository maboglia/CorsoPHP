...
["nome"=>"alunno", "cognome"=>"assente",	"email" => ""		], 
["nome"=>"alunno", "cognome"=>"assente",	"email" => ""		], 
...
//eseguo la query, memorizzo il risultato in una variabile ($result)
$result = mysqli_query($mysqliConnection, $sql);



//ciclo la variabile per farmi ritornare i risultati riga per riga ($row)
while ($row = mysqli_fetch_object($result)){
	$studentiDB[] = array(	"nome"=>$row->nome, "cognome"=>$row->cognome,"email" => $row->email );
}
...
foreach ($studentiDB as $studente) {
	array_push($elencoStudenti, 
	new Studente($studente['nome'], $studente['cognome'], $studente['email'])
	);
}
